...
describe('one binary, src/main.rs', async () => {
  beforeEach(() => {
    jest.mock('fs-extra', () =>
      Object.assign(jest.genMockFromModule('fs-extra'), {
        readdir: jest.fn(async _ => ['main.rs']),
...
describe('two binaries, src/main.rs, src/bar.rs', async () => {
  beforeEach(() => {
    jest.mock('fs-extra', () =>
      Object.assign(jest.genMockFromModule('fs-extra'), {
        readdir: jest.fn(async _ => ['main.rs', 'bar.rs']),
        exists: jest.fn(async p => p.endsWith('main.rs')),
      }),
    );
    inferCargoBinaries = require('../inferCargoBinaries');
...
describe('one named binary, no main.rs', async () => {
  beforeEach(() => {
    jest.mock('fs-extra', () =>
      Object.assign(jest.genMockFromModule('fs-extra'), {
        readdir: jest.fn(async _ => ['bar.rs']),
      }),
    );
    inferCargoBinaries = require('../inferCargoBinaries');
  });
